...
// SPEC-245-224 — icon mutation is admin-only on the SHARED catalog: a non-admin
// PUT/DELETE of a service icon is 403 (covered by shared_catalog_test.go). The
// retired v9 "non-admin manages the icons on their OWN service" test is gone;
// the admin icon-mutation tests below prove the admin path is unchanged.
